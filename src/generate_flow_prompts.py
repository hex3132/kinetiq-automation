...
only generates visuals — this version explicitly tells Flow to generate
NO dialogue/voice of its own, only non-verbal sound effects, since the
real humanized voiceover is added separately afterward (voiceovers.zip).
"""
...
7. NO DIALOGUE, NO SPOKEN VOICE, NO NARRATION should be generated by Flow itself — the real voiceover is added separately afterward. Every environment/effects field must describe ONLY non-verbal sound (whooshes, mechanical clanks, ambient drone, impact hits) — explicitly write "no dialogue, no voice, ambient sound effects only" into each chunk's environment atmosphere and render_keywords so Flow doesn't generate its own competing voice track.
...
def write_flow_prompts_file(flow_prompts, script, config, out_path="output/google_flow_prompts.txt"):
    ...
    lines = [
        ...
        "3. Keep Flow's own generated ambient SFX track (it was instructed to have no voice),",
        "   just make sure it's not louder than the real voiceover.",
