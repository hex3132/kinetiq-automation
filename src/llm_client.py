...
def _call_groq(system_prompt, user_prompt, api_key, json_mode):
    ...
    for model_name in GROQ_MODEL_CANDIDATES:
        ...
        try:
            ...
            if resp.status_code in (400, 413):
                print(f"[llm_client] Groq model '{model_name}' rejected the request ({resp.status_code}: {resp.text[:200]}), trying next candidate...")
                last_error = f"{resp.status_code} for {model_name}"
                continue
            resp.raise_for_status()
            data = resp.json()
    ...
    raise RuntimeError(f"All Groq model candidates failed. Last error: {last_error}. Tried: {GROQ_MODEL_CANDIDATES}")
